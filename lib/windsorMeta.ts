...
export async function fetchMetaCampaigns(rangeKey: DateRangeKey = "month"): Promise<{ campaigns: CampaignRow[]; warnings: string[] }> {
  ...
  try {
    ...
    warnings.push(
      `Windsor.ai (Meta Ads, campañas): no se pudo consultar el histórico de 12 meses para descubrir campañas sin actividad. Detalle: ${err?.message || err}`
    );
  }

  if (byCampaign.size === 0) {
    warnings.push(`Windsor.ai (Meta Ads, campañas): no se encontró ninguna campaña conectada, ni con actividad ni sin ella, en el último año.`);
    return { campaigns: [], warnings };
  }
